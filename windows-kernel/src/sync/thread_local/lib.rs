...
#[allow(deprecated)]
pub use super::cached::{CachedIntoIter, CachedIterMut, CachedThreadLocal};
use super::thread_id;
use super::thread_id::Thread;
use super::unreachable::UncheckedResultExt;
...
pub(crate) const POINTER_WIDTH: u8 = 64;
    ...
    fn next(&mut self) -> Option<T> {
        self.raw.next_mut(&mut self.thread_local).map(|entry| {
            *entry.present.get_mut() = false;
            unsafe {
                core::mem::replace(&mut *entry.value.get(), MaybeUninit::uninit()).assume_init()
            }
        })
    }
    fn size_hint(&self) -> (usize, Option<usize>) {
...
unsafe fn deallocate_bucket<T>(bucket: *mut Entry<T>, size: usize) {
    let _ = Box::from_raw(core::slice::from_raw_parts_mut(bucket, size));
}

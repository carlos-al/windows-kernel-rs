...
impl RandomState {
    pub fn new() -> RandomState {
        static KEYS: OnceLock<(AtomicU64, AtomicU64)> = OnceLock::new();

        let (k0, k1) = KEYS.get_or_init(|| {
            (
                AtomicU64::new(Instant::now().t.as_secs()), //TODO: Proper random inputs
                AtomicU64::new(Instant::now().t.as_secs()),
            )
        });

        let k0 = 1 + k0.fetch_add(1, Ordering::SeqCst) * thread::current_id() as u64;
        let k1 = k1.load(Ordering::SeqCst) * thread::current_id() as u64;
        RandomState { k0, k1 }
    }
}
...
impl<K: Eq + Hash + Clone> HashSet<K, RandomState> {

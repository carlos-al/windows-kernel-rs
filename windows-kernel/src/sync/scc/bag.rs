    ...
    fn next(&mut self) -> Option<Self::Item> {
        while self.current_index != u32::MAX {
            ...
            if let Some(linked) = self.current_stack_entry.as_mut() {
                ...
                let stack = &self.bag.stack;

                {
                    let current_stack_entry = &mut self.current_stack_entry;
                    let current_index = &mut self.current_index;

                    stack.peek_with(|e| {
                        if let Some(e) = e {
                            let entry_mut =
                                (e as *const LinkedEntry<Storage<T, ARRAY_LEN>>).cast_mut();
                            *current_stack_entry = unsafe { entry_mut.as_mut() };
                            *current_index = 0;
                        }
                    });
                }

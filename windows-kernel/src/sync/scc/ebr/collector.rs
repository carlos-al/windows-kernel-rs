//--- This file has been modified, differing from the original github repo wvwwvwwv/scalable-delayed-dealloc ---//
use alloc::boxed::Box;
use core::panic;
...
use crate::sync::critical::sync::Lazy;
use crate::sync::scc::exit_guard::ExitGuard;
...
impl Collector {
    ...
    pub(super) fn current() -> *mut Collector {
        let local_collector = LOCAL_COLLECTOR.get_or_default();

        let mut collector_ptr = local_collector.load(Relaxed);
        if collector_ptr.is_null() {
            let anchor = COLLECTOR_ANCHOR.get_or_default();
            collector_ptr = CollectorAnchor::alloc(anchor);
...
static COLLECTOR_ANCHOR: Lazy<ThreadLocal<CollectorAnchor>> = Lazy::new(|| ThreadLocal::new());
static LOCAL_COLLECTOR: Lazy<ThreadLocal<AtomicPtr<Collector>>> = Lazy::new(|| ThreadLocal::new());
